...
void Card::on_buy_smth_triggered()
{
    ...
    int choice;
    float new_amount = 0;
    items << QString::fromUtf8("Репецития - 1500р.") << QString::fromUtf8("Яндекс+ - 59,90")  << QString::fromUtf8("Продукты - 678") << QString::fromUtf8("Подписка - 312,50") << QString::fromUtf8("Коммуналка - 3264,76");
    ...
    if (item == "Продукты - 678")
    {
        choice = 3;
    }

    if (item == "Подписка - 312,50")
    {
        choice = 4;
    }

    if (item == "Коммуналка - 3264,76")
    {
        choice = 5;
    }
    ...
    case 1:
        ...
        if (new_amount < 0)
        {
            QMessageBox::about(this, "Ошибка", "Нехватка денег, отмена покупки");
        }
        else
        {
